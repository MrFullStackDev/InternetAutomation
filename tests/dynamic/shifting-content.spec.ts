...
test.describe('Shifting Content', () => {
  test('menu variant exposes shifting menu items @regression', async ({ shiftingContentPage }) => {
    await shiftingContentPage.openVariant('menu');
    const count = await shiftingContentPage.menuItems.count();
    expect(count).toBeGreaterThan(3);
  });

  test('image variant renders an image @regression', async ({ shiftingContentPage }) => {
